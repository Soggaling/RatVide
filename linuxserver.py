...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'sogga_v27_stable'
# async_mode='eventlet' is critical for Ubuntu/Linux performance
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='eventlet')

# GLOBAL STATE: Keeps data alive across refreshes
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .ping-dot { height: 8px; width: 8px; background: var(--neon); border-radius: 50%; display: inline-block; box-shadow: 0 0 8px var(--neon); transition: opacity 0.2s; }
...
<body onkeydown="handleKeyboard(event)">
    <div class="top-nav">
        <div style="font-weight:900; letter-spacing:3px;">SOGGABOARD // v2.7</div>
    ...
    <div class="main-layout">
        ...
        <div class="right-pane">
            <div class="console-wrap">
                <div id="console-output">>> SOGGABOARD v2.7 ONLINE...</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function handleScreenClick(e) {
            ...
            if(x >= 0 && x <= 1 && y >= 0 && y <= 1) socket.emit('client_command', { action: 'click', x: x, y: y });
        }

        function handleKeyboard(e) {
            if(!controlEnabled || document.activeElement.id === 'cmd-input') return;
            const block = ["Tab", "Escape", "Control", "Alt", "Meta", " "];
            if(block.includes(e.key) || e.ctrlKey || e.metaKey) e.preventDefault();
            socket.emit('client_command', { action: 'keypress', key: e.key });
        }
        ...
        function sendAction(act) { 
            if(act.includes('stop')) { view.src = ""; document.getElementById('osd').innerText = "SIGNAL: CUT"; }
...
if __name__ == '__main__':
    # Using 0.0.0.0 ensures it's accessible on your network
